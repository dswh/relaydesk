...
| Customer-facing RAG answer quality | Add an `Ask RelayDesk` help chat backed by approved public knowledge chunks in PostgreSQL with pgvector. The credible v0 uses shallow vector-only retrieval, no evidence threshold, no follow-up retrieval context, and a permissive prompt, so multi-step, no-answer, citation, and prompt-injection cases fail in different ways. | pgvector, a pinned embedding configuration, Vercel AI SDK, Promptfoo, deterministic retrieval, permission, citation, schema, and refusal graders, plus a blinded LLM judge for correctness, groundedness, completeness, relevance, and clarity. | 100% hard-gate compliance, source recall at 5 at least 0.93, mean reciprocal rank at least 0.85, no-source accuracy at 1.00, and all judge dimension thresholds passing across three uncached runs. Stop only after two complete passes. | Maximum 10 iterations or 90 minutes with a fixed spend limit. Change one prompt or retrieval variable per iteration. Dataset, reference answers, expected sources, judge, and thresholds are immutable. | Learners see a plausible answer fail in the browser, identify whether retrieval or generation caused it, and watch separate metrics improve as answers become grounded, citable, and willing to abstain. | It demonstrates the complete RAG quality loop and makes the limits of LLM judgment explicit. A better prompt cannot repair missing evidence, and a high similarity score cannot guarantee a safe answer. | Healthy implementation on `codex/help-chat-rag-quality`: 15 of 15 deterministic cases, recall@5 1.00, MRR 1.00, no-source accuracy 1.00, 22 of 22 Promptfoo checks, and 3 of 3 browser tests passing. Frozen starting state: `codex/scenario-help-chat-rag-quality`. Live generation and judging require `AI_GATEWAY_API_KEY`. |
...
Run `pnpm dev` and open `/help/ask` for the customer-visible chat. Copy [`evaluations/help-chat/LOOP_PROMPT.md`](../evaluations/help-chat/LOOP_PROMPT.md) into the coding agent. Each evaluation creates `evaluation-results.csv` and `score-summary.csv`; import them as `Cases` and `Scorecard` in one Google Sheet. Add `AI_GATEWAY_API_KEY` before `pnpm eval:help-chat:live` to use live answer generation and the blinded judge.
