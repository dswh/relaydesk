...
For the Lighthouse session, replace both `postgres` references with `lighthouse` and use `origin/codex/scenario-lighthouse-quality`. Each participant works on a disposable run branch while the scenario remains frozen and easy to reset.
